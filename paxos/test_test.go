...
func nchosen(t *testing.T, pxa []*Paxos, seq int) int {
	count := 0
	var v interface{}
	for i := 0; i < len(pxa); i++ {
		if pxa[i] != nil {
			chosen, v1 := pxa[i].Status(seq)
			if chosen == Chosen {
				if count > 0 && v != v1 {
					t.Fatalf("chosen values do not match; seq=%v i=%v v=%v v1=%v",
						seq, i, v, v1)
				}
				count++
				v = v1
...
func waitn(t *testing.T, pxa []*Paxos, seq int, wanted int) {
	...
	nd := nchosen(t, pxa, seq)
	if nd < wanted {
		t.Fatalf("too few chosen; seq=%v nchosen=%v wanted=%v", seq, nd, wanted)
	}
}
...
func checkmax(t *testing.T, pxa []*Paxos, seq int, max int) {
	...
	nd := nchosen(t, pxa, seq)
	if nd > max {
		t.Fatalf("too many chosen; seq=%v nchosen=%v max=%v", seq, nd, max)
	}
}
...
func TestRPCCount(t *testing.T) {
	...
	// Proposer 1: 3 prep, 3 acc, 3 chooses.
	// Proposer 2: 3 prep, 3 acc, 3 prep, 3 acc, 3 chooses.
	// Proposer 3: 3 prep, 3 acc, 3 prep, 3 acc, 3 prep, 3 acc, 3 chooses.
...
func TestMany(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		...
		for seq >= 5 && nchosen(t, pxa, seq-5) < npaxos {
...
//
// a peer starts up, with proposal, after others choose.
...
func TestManyUnreliable(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		...
		// number of file descriptors.
		for seq >= 3 && nchosen(t, pxa, seq-3) < npaxos {
			time.Sleep(20 * time.Millisecond)
		}
		for i := 0; i < npaxos; i++ {
			pxa[i].Start(seq, (seq*10)+i)
...
func TestPartition(t *testing.T) {
	...
	for iters := 0; iters < 20; iters++ {
		...
		waitmajority(t, pxa, seq)
		if nchosen(t, pxa, seq) > 3 {
			t.Fatalf("too many chosen")
		}
...
func TestLots(t *testing.T) {
	...
	// repair, then check that all instances chosen.
	for i := 0; i < npaxos; i++ {
		pxa[i].setunreliable(false)
	}
	part(t, tag, npaxos, []int{0, 1, 2, 3, 4}, []int{}, []int{})

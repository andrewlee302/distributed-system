...
func TestBasic(t *testing.T) {
	...
	ck := MakeClient(kvh)
	var cka [nservers]*Client
	for i := 0; i < nservers; i++ {
		cka[i] = MakeClient([]string{kvh[i]})
	}
...
func TestDone(t *testing.T) {
	...
	for pi := 0; pi < nservers; pi++ {
		cka[pi] = MakeClient([]string{kvh[pi]})
	}
...
func TestPartition(t *testing.T) {
	...
	var cka [nservers]*Client
	for i := 0; i < nservers; i++ {
		cka[i] = MakeClient([]string{port(tag, i)})
	}
...
func randclerk(kvh []string) *Client {
...
func TestHole(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		...
		ck2 := MakeClient([]string{port(tag, 2)})
		ck2.Put("q", "q")
